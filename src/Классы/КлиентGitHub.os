...
Функция СоздатьФоркВОрганизацию(Знач ВладелецРепозитория, Знач ИмяРепозитория, Знач ОрганизацияПриемник) Экспорт

	Перем Репозиторий;

	СсылкаЗапроса = СтрШаблон("/repos/%1/%2/forks?organization=%3", ВладелецРепозитория, ИмяРепозитория, ОрганизацияПриемник);
...
Функция Заголовки()
	...
	ЗаголовкиЗапроса.Вставить("User-Agent", "oscriptgithubapi");
	ЗаголовкиЗапроса.Вставить("Authorization", СтрШаблон("token %1", ТокенАвторизации));
	ЗаголовкиЗапроса.Вставить("Content-Length", "0"); // необходим для некоторых POST запросов
