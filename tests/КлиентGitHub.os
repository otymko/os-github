...
&Тест
Процедура Тест_ПроверитьФоркРепозитория() Экспорт

	Клиент = Новый КлиентGitHub(Токен);

	Пользователь = "freeCodeCamp";
	Репозиторий = "freeCodeCamp";
	Организация = "hos-torg";

	РепозиторийФорк = Клиент.СоздатьФоркВОрганизацию(Пользователь, Репозиторий, Организация);
	Ожидаем.Что(РепозиторийФорк, "Форк репозитория в организацию по пользователю, репозиторию и организации 
	|(может падать, т.к. нет доступа к данной организации)").Заполнено();

КонецПроцедуры

&Тест
Процедура Тест_ПроверитьПредоставлениеДоступа() Экспорт

	Клиент = Новый КлиентGitHub(Токен);

	КомуПредоставитьДоступ = "k2589";
	Репозиторий = "freeCodeCamp";
	Владелец = "hos-torg";

	Результат = Клиент.ПредоставитьДоступКРепозиторию(Владелец, Репозиторий, КомуПредоставитьДоступ);

	Ожидаем.Что(Результат.ЕстьОшибки, "Отсутствуют ошиюки при предоставлении доступа к репозиторию").Равно(Ложь);
